...
SELECT * 
    INTO v_customers
    FROM Customer
    dbms_output.put_line SELECT( v_customers.Customer_id |''| v_customers.customer_Name |''| v_customers.customer_Tel);
END;

--2 

CREATE OR REPLACE PROCEDURE PS_Customer_Products (v_customers_id Orders.Customer_id%type) IS
